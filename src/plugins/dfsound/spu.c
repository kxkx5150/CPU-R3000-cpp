...
INLINE void StoreInterpolationVal(int ch, int fa)
{
    if (s_chan[ch].bFMod == 2)    // fmod freq channel
        ...
        } else if (iUseInterpolation == 1)    // simple interpolation
        {
            s_chan[ch].SB[28] = 0;
            s_chan[ch].SB[29] =
                s_chan[ch].SB[30];    // -> helpers for simple linear interpolation: delay real val for two slots, and
                                      // calc the two deltas, for a 'look at the future behaviour'
                    ...
                    while (s_chan[ch].spos >= 0x10000L) {
                        if (s_chan[ch].iSBPos == 28)    // 28 reached?
                        {
                            ...
                            if (flags & 1)    // 1: stop/loop
                            {
                                ...
                                if (flags != 3 ||
                                    s_chan[ch].pLoop == NULL)    // PETE: if we don't check exactly for 3, loop hang ups
                            ...
                            if (bIRQReturn)    // special return for "spu irq - wait for cpu action"
                            {
                                ...
                                if (iUseTimer != 2) {
                                    ...
                                    while (iSpuAsyncWait && !bEndThread && timeGetTime_spu() < dwWatchTime) {
                                        printf("sleep\n");
                                        usleep(1000L);
                                    }
                                } else {
