...
void InitSDLJoy()
{
    ...
    printf("InitSDLJoy devnum=%d\n", SDL_NumJoysticks());

    for (i = 0; i < 2; i++) {
        if (g.cfg.PadDef[i].DevNum >= 0) {
            g.PadState[i].JoyDev = SDL_JoystickOpen(g.cfg.PadDef[i].DevNum);
            printf("joydev %d = %p\n", i, g.PadState[i].JoyDev);
            if (g.PadState[i].JoyDev) {
                printf("numAxes %d\n", SDL_JoystickNumAxes(g.PadState[i].JoyDev));
                printf("numButtons %d\n", SDL_JoystickNumButtons(g.PadState[i].JoyDev));
                printf("numHats %d\n", SDL_JoystickNumHats(g.PadState[i].JoyDev));
            }
        } else {
