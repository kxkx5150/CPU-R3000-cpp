...
////////////////////////////////////////////////////////////////////////
void GPUdisplayFlags(unsigned long dwFlags)    // some info func
{
    // dwCoreFlags=dwFlags;
    // BuildDispMenu(0);
}
...
void updateDisplayIfChanged(void)    // UPDATE DISPLAY IF CHANGED
{
    ...
    ChangeDispOffsetsX();
    if (iFrameLimit == 2)
        SetAutoFrameCap();    // -> set it
    if (UseFrameSkip)
        updateDisplay();    // stupid stuff when frame skipping enabled
}
////////////////////////////////////////////////////////////////////////
#ifndef _MACGL
#include "draw.h"
...
////////////////////////////////////////////////////////////////////////
void GPUupdateLace0(void)    // VSYNC
{
    if (!(dwActFixes & 1))
        lGPUstatusRet ^= 0x80000000;    // odd/even bit
    CheckFrameRate();
}
void GPUupdateLace1(void)    // VSYNC
{
...
#ifndef _MACGL
    if (bChangeWinMode)
        ChangeWindowMode();    // toggle full - window mode
#endif
    bDoVSyncUpdate = FALSE;    // vsync done
}
////////////////////////////////////////////////////////////////////////
...
uint32_t GPUreadStatus(void)    // READ STATUS
{
    if (dwActFixes & 1) {
        ...
        if ((iNumRead++) == 2) {
            iNumRead = 0;
            lGPUstatusRet ^= 0x80000000;    // interlaced bit toggle... we do it on every 3 read status... needed by
                                            // some games (like ChronoCross) with old epsxe versions (1.5.2 and older)
        }
    }
...
////////////////////////////////////////////////////////////////////////
__inline static void FinishedVRAMWrite(void)
{
    /*
    // NEWX
...
__inline static void FinishedVRAMRead(void)
{
    // Set register to NORMAL operation
...
__inline static BOOL CheckForEndlessLoop(unsigned long laddr)
{
...
////////////////////////////////////////////////////////////////////////
long GPUfreeze(uint32_t ulGetFreezeData, GPUFreeze_t *pF)
{
    //----------------------------------------------------//
...
unsigned char cFont[10][120] = {
    ...
    {0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00,
     ...
     0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa}};
////////////////////////////////////////////////////////////////////////
inline void PaintPicDot(unsigned char *p, unsigned char c)
{
    if (c == 0) {
        *p++ = 0x00;
        *p++ = 0x00;
...
void GPUshowScreenPic(unsigned char *pMem)
{
   ...
   CreatePic(pMem);                                      // create new pic... don't free pMem or something like that...
   just read from it
   */
}
