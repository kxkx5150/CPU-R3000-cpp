...

    {
        ...
        if ((_ticks_since_last_update > TicksToWait) || (curticks < lastticks)) {
            ...
            while (Waiting) {
                ...
                if ((_ticks_since_last_update > TicksToWait) || (curticks < lastticks) || tickstogo < overslept) {
                    Waiting     = FALSE;
                    lastticks   = curticks;
                    overslept   = _ticks_since_last_update - TicksToWait;
                    TicksToWait = dwFrameRateTicks - overslept;
                ...
                if (tickstogo >= 30 && !(dwActFixes & 16)) {    // printf("fps sleep\n");
...
void FrameSkip(void)
{
    ...
    if (!dwLaceCnt)
        return;    // important: if no updatelace happened, we ignore it completely
    ...
    {                               // we check now, if some limitation is needed, or a new skipping has to get started
        ...
        if (_ticks_since_last_update > dwWaitTime)    // hey, we needed way too long for that frame...
        {
            ...
            {                     // and decide after, if we need to do more
                ...
                if (iNumSkips > MAXSKIP)
                    iNumSkips = MAXSKIP;    // -> well, somewhere we have to draw a line
            }
...
void InitFPS(void)
{
    if (!fFrameRate)
        fFrameRate = 200.0f;
    if (fFrameRateHz == 0)
        fFrameRateHz = fFrameRate;    // set user framerate
