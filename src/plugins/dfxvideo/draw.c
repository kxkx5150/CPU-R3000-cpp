...
void     CreateDisplay(void)
{
    if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_JOYSTICK | SDL_INIT_AUDIO) < 0) {
        ...
        sdlswnd     = SDL_CreateWindow("", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, iResX, iResY, 0);
        sdl_display = SDL_GetWindowSurface(sdlswnd);
        sdl_ximage  = SDL_CreateRGBSurface(0, iResX, iResY, depth, 0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000);
    }
}
...
#else
                destpix[row] = 0xff000000 | (lr << 16) | (lg << 8) | (lb);
#endif
                pD += 3;
            }
        }
...
#else
                destpix[row] = (((s << 19) & 0xf80000) | ((s << 6) & 0xf800) | ((s >> 7) & 0xf8)) | 0xff000000;
#endif
                // printf("blt d %x\n", destpix[row]);
            }
        }
...
void DoBufferSwap(void)
{
    ...
    SDL_UpdateWindowSurface(sdlswnd);
    dstrect.x = 0;
    dstrect.y = 0;
    dstrect.w = iResX;
    dstrect.h = iResY;
